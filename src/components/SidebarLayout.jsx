...
const SidebarLayout = ({ children }) => {
  ...
  // Route change hote hi mobile menu band ho jaye
  useEffect(() => {
  ...
  return (
    <div className="flex min-h-screen bg-[#050505] text-white selection:bg-indigo-500/30">
      ...
      {/* MOBILE OVERLAY: Sirf tab dikhega jab menu khula ho */}
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div 
            initial={{ opacity: 0 }}
      ...
      {/* ASIDE: Added transition and mobile positioning */}
      <aside className={`
      ...
      `}>
        ...
        <div className="p-10 flex justify-between items-center">
          ...
          {/* Mobile Close Button */}
          <button onClick={() => setIsMobileMenuOpen(false)} className="lg:hidden text-gray-500 p-2">✕</button>
        </div>
        ...
        <nav className="flex-1 px-6 space-y-2">
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.name} to={item.path}>
                ...
                >
                  {isActive && (
                    <motion.div layoutId="activeTab" className="absolute left-0 w-1 h-6 bg-indigo-500 rounded-r-full shadow-[0_0_15px_#4f46e5]" />
                  )}
          ...
          <div className="pt-4">
            ...
            >
              ...
              <motion.span animate={{ rotate: isSuggestionsOpen ? 180 : 0 }} className="text-[10px]">▼</motion.span>
            </button>
        ...
        <div className="p-8 border-t border-white/5">
          <button onClick={handleLogout} className="w-full group flex items-center justify-between px-6 py-4 rounded-2xl bg-white/[0.03] border border-white/5 hover:bg-red-500/10 hover:border-red-500/50 transition-all">
            <span className="text-[9px] font-black uppercase tracking-widest text-gray-500 group-hover:text-red-500">Terminate Link</span>
            <svg className="w-4 h-4 text-gray-700 group-hover:text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
          </button>
        </div>
      </aside>

      {/* MAIN: Margin adjust for mobile */}
      <main className="flex-1 lg:ml-80 min-h-screen relative flex flex-col">
        
        {/* HEADER: Added Hamburger for mobile */}
          ...
          <div className="flex items-center gap-3 md:gap-6">
            ...
            <div className="relative w-10 h-10 md:w-12 md:h-12 flex items-center justify-center">
              ...
              <div className="relative w-8 h-8 md:w-10 md:h-10 bg-[#0d0d0d] border border-white/10 rounded-xl rotate-45 flex items-center justify-center">
                <span className="text-xs font-black text-white -rotate-45">{user.initial}</span>
              </div>
            </div>
          </div>
        </header>
